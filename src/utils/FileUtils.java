...
public class FileUtils {
    ...
    public static String listAndPickFileFromDir(String dirPath){
        ...
        Output.output("Please enter number of file to load: ");
        for(int i = 0; i < files.length; ++i){
            if(files[i].isFile())
                // Adding 1 to avoid 0-indexed UI. getName to chop off full path.
                Output.output((i + 1) + ") " + files[i].getName());
        }
    ...
    public static List<String> getFilesStartingWith(String word, String directoryPath) {
        ...
        for (File file : files) {
            if (file.isFile() && file.getName().startsWith(word)) {
                matchingFiles.add(file.getAbsolutePath());
            }
        }

        return sortPaths(matchingFiles);
    }

    public static String listAndPickFileStartingWith(String word, String dirPath) {
        List<String> matchingFiles = getFilesStartingWith(word, dirPath);
        if (matchingFiles.isEmpty()) {
            throw new IllegalStateException("No files found starting with " + word);
        }
        Output.output("Please enter number of file to load: ");
        for (int i = 0; i < matchingFiles.size(); ++i) {
            Output.output((i + 1) + ") " + new File(matchingFiles.get(i)).getName());
        }
        int fileSelection = Input.readIntInRange(1, matchingFiles.size());
        return matchingFiles.get(fileSelection - 1);
    }
}
